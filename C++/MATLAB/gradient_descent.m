...
f_change=(x-1)^2;             %初始化差值
f_current=(x-1)^2;            %计算当前函数值
ezplot(@(x,f)f-(x-1).^2)       %画出函数图像
axis([-2,3,-0.2,7])       %固定坐标轴
hold on
while f_change>0.000000001                %设置条件，两次计算的值之差小于某个数，跳出循环
    x=x-step*2*(x-1);                         %-2*x为梯度反方向，step为步长，！最速下降法！
    f_change = f_current - (x-1)^2;           %计算两次函数值之差
    f_current = (x-1)^2 ;                     %重新计算当前的函数值
